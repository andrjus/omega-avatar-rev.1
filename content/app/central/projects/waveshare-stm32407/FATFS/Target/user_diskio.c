...
)
{
  /* USER CODE BEGIN READ */
	  ...
	  NAND_Address.Plane = 0;
		NAND_Address.Block = sector;
		NAND_Address.Page = 1;
		return BSP_NAND_Read_Page(&hnand1, &NAND_Address, (uint8_t *)buff, count) == HAL_OK? RES_OK : RES_ERROR;
  /* USER CODE END READ */
}
  ...
  /* USER CODE HERE */
	int n;
	  ...
	  NAND_Address.Plane = 0;
		...
		static uint8_t buf2[2048]={5,5,5};
		
		BSP_NAND_Erase_Block(&hnand1, &NAND_Address);		
		BSP_NAND_Write_Page(&hnand1, &NAND_Address, (uint8_t *)buff, count);// == HAL_OK? RES_OK : RES_ERROR;
		BSP_NAND_Read_Page(&hnand1, &NAND_Address, (uint8_t *)buf2, 1);
  ...
  /* USER CODE BEGIN IOCTL */
	...
	switch (cmd){
		...
		case CTRL_SYNC:		 /* Complete pending write process (needed at _FS_READONLY == 0) */
			if (HAL_NAND_Read_Status(&hnand1) == NAND_READY) res = RES_OK;
			break;
